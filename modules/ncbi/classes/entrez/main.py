...
### BioPython Entrez Wrapper Methods ########################################################################################
	...
	def esearch(cls, esearch_params=None, return_handle=False):
		...
		try:
			...
			raise Exception(f'db: {db}; retmx: {retmax}; retstart: {retstart}; term: {term}; idtype: {idtype}')
	...
	def esummary(cls, esummary_params=None, as_json=False):
		...
		esummary_handle = None
		while esummary_handle == None:
			try:
				esummary_handle = get_esummary_handle(
					db=db,
					id=id,
					retstart=retstart,
					retmax=retmax,
